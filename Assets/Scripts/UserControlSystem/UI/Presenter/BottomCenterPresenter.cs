...
public class BottomCenterPresenter : MonoBehaviour
{
    ...
    private void Init(BottomCenterModel model, BottomCenterView view)
    {
        ...
        if (unitProducer != null)
        {
            ...
            _productionQueueRemoveCt = unitProducer.Queue.ObserveRemove().Subscribe(removeEvent => view.SetTask(null, removeEvent.Index));
            _productionQueueReplaceCt = unitProducer.Queue.ObserveReplace().Subscribe(replaceEvent => view.SetTask(replaceEvent.NewValue, replaceEvent.Index));
